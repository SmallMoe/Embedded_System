...
#define EVENT_MAX_STORAGE        5 // 5 most recent events only
...
char codeSequence[NUMBER_OF_KEYS] = {'1', '8', '0', '5'}; //Password '1805'
...

{
    ...
    // When Alarm is ON, keypad handle code entry
    ...
    if (key >= '0' && key <= '9') {
        if (keyCount < NUMBER_OF_KEYS) {
            ...
            keyCount++;
            char str[20];
            sprintf(str, "Key pressed: %c\r\n", key); //Printing which key was pressed on matrix keypad
            uartUsb.write(str, strlen(str));
...

{
    ...
    if ( alarmState ) {
        ...
        buzzer = OFF; //Actively high so when alarm state is true, buzzer is OFF which is physically on. (Connected to 5V)
        alarmLed = ON;
...

{
    ...
    if( uartUsb.readable() ) {
        ...
        case 'C':
            ...
            // lm35TempC should be updated by sensor reading function
...
// Handling Matrix keypad debounce to prevent noise and only take intended input
